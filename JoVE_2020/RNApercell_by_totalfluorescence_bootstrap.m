...
for ti = 1:length(timePoints)
    ...
    Xans(ti,3) = var(bootint(:,1))/mean(bootint(:,1));  %fano factor
    Xans(ti,4) = mean(bootint(:,2)); 
    Xans(ti,5) = std(bootint(:,2));
    Xans(ti,6) = var(bootint(:,2))/mean(bootint(:,2));  %fano factor
    Xans(ti,7) = nboot*length(find(cellNumperday(ti,:)>0)); 
end
